        ...
        if self.__waiting_customers:
            ...
            for customer in self.__waiting_customers:
                ...
                if customer.get_required_waiting_time() <= customer.get_waited_time()  and not self.__kitchen.all_the_cooks_cooking():
                    ...
                    if table_num is None and self.__table_manager.is_table_full():
                        #들어올때는 테이블이 꽉 차지는 않았지만, 대기 손님들에 의해 테이블이 꽉 차게되었다.
                        minimum_cooking_time = min(self.__kitchen.get_cooks_current_cooking_time())
                        customer.set_required_waiting_time(customer.get_required_waiting_time() + minimum_cooking_time)
                        print(f"{customer.get_customer_number()}번 손님 추가 대기")
                        continue
