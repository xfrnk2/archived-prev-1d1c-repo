...
class Restaurant:
    ...
    def customer_entrance(self, customer: Customer):
        ...
        print(result)
        print(f"1111요리사에게 배당되기까지의 시간{result}")

        #####
        print(f"2222요리 배당 대기시간 {customer.get_required_waiting_time()}")

        print(
            customer.get_required_waiting_time() + customer.get_food_cooking_time() + customer.get_food_eating_time())
